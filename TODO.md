...
Follow-up pass over the open P1 findings from the 0.2.4 audit wave. Five of the eight previously-open P1 items turned out to be **already fixed at 0.2.4 release time** but the changelog hadn't been updated to reflect the as-committed code; the remaining two genuine parser gaps are closed below, the parity suite is regenerated, and the regeneration surfaced one new soundness finding (promoted to P1).
